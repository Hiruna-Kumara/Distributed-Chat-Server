...
// * The timer thread waits for 7 seconds to receive a response. If it receives
// an
...
// leaderFlag = true;
// System.out.println("INFO : Server s" +
// LeaderState.getInstance().getLeaderID()
// + " is selected as leader! ");

// LeaderState.getInstance().resetLeader(); // reset leader lists when newly
// elected
...
// System.out.println("INFO : Received OK but coordinator message was not
// received");
...
// destServer);
// // System.out.println( "INFO : Sent heartbeat to leader s" +
// // destServer.getServerID() );
// }
// }
...
// sendElectionRequest();
// } catch (Exception e) {
// System.out.println("WARN : Server has failed, election request cannot be
// processed");
// }
// break;
...
// * If the server has failed then a message is displayed to indicate the
// failure.
// */
// public static void sendCoordinatorMsg() {
// int numberOfRequestsNotSent = 0;
// for (String key : ServerState.getInstance().getOtherServers().keySet()) {
// if (Integer.parseInt(key) != ServerState.getInstance().getSelfID()) {
// Server destServer = ServerState.getInstance().getOtherServers().get(key);
...
// }
// }
// if (numberOfRequestsNotSent ==
// ServerState.getInstance().getOtherServers().size() - 1) {
...
// try {
// Server destServer =
// ServerState.getInstance().getOtherServers().get(sourceID);
// MessageTransfer.sendServer(
...
// } catch (Exception e) {
// System.out.println("INFO : Server s" + sourceID + " has failed. OK message
// cannot be sent");
// }
// }

// /**
// * The sendElectionRequest() method sends an election request to all the
// servers
// * with higher IDs
// */
...
// destServer);
// System.out.println("INFO : Sent election request to s" +
// destServer.getServerID());
// } catch (Exception e) {
// System.out.println("WARN : Server s" + destServer.getServerID() +
...
// ServerState.getInstance().getNumberOfServersWithHigherIds()) {
